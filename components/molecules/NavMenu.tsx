...
export default function NavMenu({ onOpen }: Props) {
  ...
  return (
    ...
    >
      ...
      <nav className={`mt-4 flex flex-col gap-3 transition-all duration-300 overflow-hidden ${open ? 'opacity-100 max-h-60' : 'opacity-0 max-h-0'}`}>
        {links.map((item) => (
          <button
            key={item}
            onClick={() => onOpen(item.toLowerCase())}
